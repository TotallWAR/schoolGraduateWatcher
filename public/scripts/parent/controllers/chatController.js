angular.module('chatParentController', [])
    .controller('chatParentCtrl', ['$scope', 'socketSrvc', 'chatSrvc', 'usersSrvc', '$sce', function($scope, socketSrvc, chatSrvc, usersSrvc, $sce) {
        ...
        socketSrvc.on('init', function(data) {
            chatSrvc.getTeachers().then((data) => {
                $scope.user = data.data.user;
                $scope.name = data.data.user.firstName + " " + data.data.user.lastName;
                $scope.teachers = data.data.teachers;
                $.each($scope.teachers, (index, item) => {
                    usersSrvc.getUserById(item.userId).then((data) => {
        ...
        $scope.changedValue = (teacher) => {
            ...
            if (teacher !== null) {
                ...
                chatSrvc.getMessages($scope.user._id, $scope.currentTeacher.userId).then((data) => {
        ...
        $scope.sendMessage = function() {
            ...
            var message = {
                ...
                parentId: $scope.user._id,
                teacherId: $scope.currentTeacher.userId,
